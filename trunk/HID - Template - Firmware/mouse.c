...
static WORD valori[12]={0};
WORD SOGLIE[12]={0,0,0,0,200,200,400,612,612,400,800,800};
...
void ParseFeatureReport(void); /*IVANO: fa il parsing del feature report ricevuto*/
...
//	to it in HardwareProfile.h.
	...
	TRISC  = 0b11101111;	/* Port C pins as inputs */
	TRISB  = 0b11110000;	/* Port B pins as inputs */
	LATC   = 0b00000000;	/* LED On */
	ANSEL  = 0b11111000;	/* AN3-7 as analog pins */
	ANSELH = 0b00001111;	/* AN8-11 as input pins */
	
	/* Ensure ADC is on  */
	ADCON0 = 0b00010001;
	/* Voltage references = Vdd for positive and Vss for negative */
	ADCON1 = 0b00000000;
...
void invia_report()
{
		...
		UsbInHandle = HIDTxPacket(HID_EP, (BYTE*)hid_report_in, IN_REPORT_SIZE);
		while (HIDTxHandleBusy(UsbInHandle));

}

static const BYTE pinMap[] = {0, 0, 0, 0, 0, 1, 2, 3, 6, 7, 4, 5};
INT tris_sel[2] = {&TRISC,&TRISB};
INT lat_sel[2] = {&LATC,&LATB};
void wait_a_bit()
{
	int i = 50;
	while(i--);
...
WORD SampleChannel(AnalogPins ch)
{
	
	AnalogPins chargePin;
	...
	//carico a 1
	*((BYTE *)lat_sel[chargePin/10]) |= (1<<pinMap[chargePin]);
	*((BYTE *)tris_sel[chargePin/10]) &= ~(1<<pinMap[chargePin]);
	//carico a 0
	*((BYTE *)lat_sel[ch/10]) &= ~(1<<pinMap[ch]);
	*((BYTE *)tris_sel[ch/10]) &= ~(1<<pinMap[ch]);
...
{  
	...
	for(i=0; i<12;i++)valori[i]=0;
	for(i=0;i<8;i++)
	{
		for (pin = 4; pin <12; pin++)
			valori[pin] += SampleChannel(pin);
	}
	...
	if(valori[LEFT]<512)
		hid_report_in[1]-=sensibilita,something=1;
	if(valori[RIGHT]<512)
		hid_report_in[1]+=sensibilita,something=1;
	if(valori[UP]<400)
		hid_report_in[2]-=sensibilita,something=1;
	if(valori[DOWN]<512)
		hid_report_in[2]+=sensibilita,something=1;
	if(valori[WHEELUP]<512)
	...
	if(checked == 0)
	{
		...
		if(valori[TASTO_DX]<512 != (hid_report_in[0]&2))
			hid_report_in[0]^=2,something=1;	
	}else
	{
		if(valori[TASTO_SX]<512 != (hid_report_in[0]&2))
			hid_report_in[0]^=2,something=1;
		if(valori[TASTO_DX]<512 != (hid_report_in[0]&1))
			hid_report_in[0]^=1,something=1;	
	}
	if(something)
		invia_report();
...
/******************************************************************************
 * Function:        void GetReportHandler(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        Send a feature report to the host
 *
 * Note:            
 *
 *****************************************************************************/
void UserGetReportHandler(void)
{
		...
		//Prepara il feature report
		FeatureReportOut[0]=checked;
		FeatureReportOut[1]=sensibilita/2;
	...
	{
		USBEP0SendRAMPtr(FeatureReportOut, FEATURE_SIZE, USB_EP0_NO_OPTIONS);
	}

}

/******************************************************************************
 * Function:        void SetReportHandler(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        Get a feature report from the host
 *
 * Note:      
 *
 *****************************************************************************/
...
{
	if (SetupPkt.W_Value.byte.LB == 0 && SetupPkt.wLength == FEATURE_SIZE)
	{
		USBEP0Receive(FeatureReport, FEATURE_SIZE, &ParseFeatureReport);
	}
}
 ...
 * Side Effects:    None
 *
 * Overview:        Parse feature report received from the host
 *
 * Note:            
 *
 *****************************************************************************/
void ParseFeatureReport(void)
{
